#!/bin/bash
# Modified from (primarily to add "${KC_HTTP_PORT}"):
...
# Send a GET request to /health/ready via the opened socket and write to the
# temporary file `&3`
...
myexit="$?" # Note: `myexit` must be kept with previous line
